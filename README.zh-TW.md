...
目前版本：0.2.8 | 協議版本：`mup/2026-03-17`
...
Claude 透過 function call 建立完整簡報 — 含圖表、表格、主題切換。

### PDF to Presentation — AI 讀論文、做簡報、上台報告

[![PDF to Presentation Demo](https://img.youtube.com/vi/GviEqEq88b4/maxresdefault.jpg)](https://youtu.be/GviEqEq88b4)

Claude 讀取 AlexNet 論文 PDF、撰寫 Markdown 筆記、與使用者協作截取圖表、建立 19 張投影片（含圖表與表格），最後用面板內閱讀模式逐頁簡報 — 全程語音旁白。

### Sound Pad — AI 從零作曲

[![Sound Pad Demo](https://img.youtube.com/vi/e5JqRT9t5vU/maxresdefault.jpg)](https://youtu.be/e5JqRT9t5vU)

16 個瀏覽器合成樂器，零取樣。Claude 逐層堆疊 electro swing 曲目，旁白透過事件系統精確同步每個段落轉換。
...
- **Chat** — 內建，隨時可用
- **Slides** — 簡報編輯器，含圖表、表格、主題、閱讀模式、匯出（[示範](https://youtu.be/GKeDc1DMLH0)）
- **PDF Reader** — PDF 閱覽器，支援頁面文字擷取與區域選取/截圖
- **Markdown** — Markdown 工作區，支援標註與文件管理
- **Sound Pad** — 16 鍵合成器，含音序器、獨立音量、事件系統（[示範](https://youtu.be/e5JqRT9t5vU)）
- **Voice** — 語音合成與辨識（Jarvis 模式）
- **Progress** — 任務進度追蹤
- 更多範例在 [`archive/examples/`](archive/examples/)（像素畫、遊戲、生產力工具等）
