...
| **Progress reporting** — intermediate updates during a call | Forces hosts to decide how and where to forward progress. | Use `updateState()` to update the visual UI during long operations. The function call stays simple: request in, response out. Additionally, MUPs can declare a `notifications` field in the manifest to request immediate channel push, polling, or silent modes — giving hosts a hint on how urgently state changes should reach the LLM. |
