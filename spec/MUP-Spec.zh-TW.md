...
- **一個函式，兩個入口。** manifest 中宣告的函式，LLM 可以當 tool 呼叫，使用者也可以透過 UI 觸發。同一份實作、同一個結果。
...
mup.registerFunction('renderChart', async (params, source) => {
  // params = inputSchema 定義的參數
  // source = "llm" 或 "user" — 誰呼叫的
...
向 host 要求更多（或更少）的 grid 空間。回傳 promise，包含實際分配結果。
