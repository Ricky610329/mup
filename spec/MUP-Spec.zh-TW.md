...
| `multiInstance` | boolean | `false` | 允許此 MUP 的多個實例。 |
| `notifications` | object | — | 預設通知層級設定。包含 `level`（`"immediate"` \| `"notify"` \| `"silent"`）和 `overridable`（boolean）。 |
...
MUP 採用分層信任模型。**Host 是唯一的權限守門員** — MUP 對 sandbox 之外的一切沒有直接存取權。
...
LLM / 使用者          ← 最高信任。決定策略。
     ↓ 設定權限
Host（執行環境）       ← 守門員。執行策略，中介所有存取。
     ↓ 批准 / 拒絕
MUP（沙箱化）          ← 零信任。任何事都必須請求。
```

### 原則

1. **MUP 是不受信任的。** MUP 在隔離容器中執行（例如 sandboxed iframe）。它無法直接存取檔案系統、網路或其他 MUP。

2. **所有能力都經過 host。** MUP 使用 `mup.system(action, params)` 請求 host 提供的服務。Host 根據每個 MUP 的權限決定是否執行請求。

3. **權限是被授予的，不是自行宣告的。** Manifest 的 `permissions` 欄位是「請求」，不是「授權」。Host 可以拒絕或過濾它認為不安全的權限。

4. **每個 MUP 獨立隔離。** 每個 MUP 有自己的權限範圍。MUP A 的權限不影響 MUP B。

5. **預設為 session 範圍。** 權限授予在 session 結束後應清除，除非 host 明確選擇持久化。

### Host 的職責

| 職責 | 說明 |
|------|------|
| **沙箱執行** | 在隔離容器中執行每個 MUP，預設無任何能力 |
| **請求中介** | 攔截所有 `system/request` 呼叫，檢查權限，然後執行或拒絕 |
| **權限範圍管理** | 維護每個 MUP 的權限表（如允許的檔案路徑、允許的網域） |
| **速率限制** | 限制每個 MUP 的 system request 頻率（如每秒最多 20 次）防止濫用 |
| **輸入驗證** | 在執行前驗證 MUP 傳來的所有路徑、URL 和參數 |

### MUP 的職責

| 職責 | 說明 |
|------|------|
| **宣告需求** | 在 `manifest.permissions` 中列出需要的權限 |
| **處理拒絕** | 如果 `mup.system()` 回傳錯誤，優雅降級 — 不要崩潰 |
| **最小請求** | 只請求你確實需要的能力 |

### `system/request` 安全流程

```
MUP 呼叫: mup.system('readFile', { path: '/some/file.md' })
  ↓
Host 檢查:
  1. 'readFile' 是已知的 action 嗎？（未知 → error -32601）
  2. 這個 MUP 被允許存取這個路徑嗎？（拒絕 → error）
  3. MUP 是否在速率限制內？（超過 → error）
  4. 路徑是否合法？（path traversal 攻擊 → error）
  ↓
Host 執行 action 並回傳結果。
```

### Spec 定義的 vs. Host 決定的

| Spec 定義（協議層） | Host 決定（實作層） |
|---------------------|---------------------|
| `mup.system()` 作為通用請求機制 | 提供哪些 action（readFile、fetchUrl 等） |
| Manifest `permissions` 作為請求欄位 | 實際授予哪些權限 |
| 每個 MUP 必須隔離的要求 | 如何實現隔離（iframe、VM、container） |
| 被拒絕時優雅處理錯誤 | 速率限制、路徑限制、網域白名單 |

> **給 host 實作者的提示：** 具體的 system actions（如 `readFile`、`writeFile`、`scanDirectory`、`fetchUrl`）是實作選擇，不是協議要求。不同的 host 可以根據其環境提供不同的能力。瀏覽器擴充套件 host 可能提供 `fetchUrl` 但不提供 `readFile`。本地開發 host 可能兩者都提供。生產環境 host 可能都不提供，而完全依賴 LLM 中介存取。
